...
class Gui(VacuumEnvironment):
    ...
    def execute_action(self, agent, action):
        ...
        elif("XYStatusReflexAgentProgram" not in str(agent.program)):
            agent.bump = False
            if action == 'TurnRight':
        ...
        elif("XYStatusReflexAgentProgram" in str(agent.program)):
            agent.bump = False
            if action == 'TurnRight':
                agent.direction += Direction.R
                ...
                if (agent.direction.move_forward(agent.location) in agent.pos):
                    agent.bump = True
                else:
                    agent.bump = self.move_to(agent, agent.direction.move_forward(agent.location))
        ...
        if(agent.location not in agent.pos and agent.location != old_possition):
            agent.pos.append(old_possition)
...
def XYStatusReflexAgentProgram(percept):
    """The modified SimpleReflexAgentProgram for the GUI environment."""
